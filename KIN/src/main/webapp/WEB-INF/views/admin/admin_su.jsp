...
.mid4_btn2, .mid4_btn3, .mid4_btn4:hover {
   cursor: pointer;
}
...
$(document).ready(function() {
	...
	// 증빙서류 클릭
	$("tbody").on("click", ".mid4_btn1", function() {
		$("#no").val($(this).parent().parent().attr("no"));
		
		$("#schGbn").val($("#oldGbn").val());
		$("#schTxt").val($("#oldText").val());
		
		$("#searchForm").attr("action", "adminSuAtt");
		$("#searchForm").submit();
	})
...
function drawList(list) {
	...
	for(var data of list) {
		...
		html += "<td>"; 
		html += "<div class=\"mid4_btn1\">증빙서류</div>";
		html += "</td>";
		html += "<td>";
		if(data.APPV == "2") {
			html += "<div class=\"mid4_btn2\">대기중</div>";
		}
		if(data.APPV == "0") {
			html += "<div class=\"mid4_btn3\">승 인</div>";
		}
		if(data.APPV == "1") {
			html += "<div class=\"mid4_btn4\">불 가</div>";
		}
		html += "</td>";
		html += "</tr>";
	}
...
<input type="hidden" id="oldText" value="${param.oldText}">
    ...
    <div class="mid">
    	...
    	<div class="mid2">
			...
			<span class="GBN">홈 > 관리자 페이지 > 회원 관리 > 원장</span>
    	</div>
    	<div class="mid3">
    		<div class = "mid3_1">원장 목록</div>
    	</div>
    		...
    		<form action="#" id="searchForm" method="POST">
		    	...
		    	<div class="search_b">
	            	<input type ="text" class="b_txt" name="searchText" id="schText" 
	            	       placeholder="원장 검색" value="${param.searchText}">
		    	</div>
